...
static const char s_sd_frag[] =
    ...
    "    vec3 stars = vec3(0.0);\n"
    "\n"
    "    /* Galactic latitude density modulation:\n"
    "     * More stars near screen center (galactic plane), fewer at poles.\n"
    "     * Gaussian falloff from y=0.5 mimics MW disc concentration. */\n"
    "    float gal_lat = abs(v_uv.y - 0.5) * 2.0; /* 0=plane, 1=pole */\n"
    "    float gal_density = exp(-gal_lat * gal_lat * 2.0); /* Gaussian */\n"
    "    float density_mod = 0.4 + gal_density * 0.6; /* 40-100% */\n"
    "\n"
    "    /* 3 layers at different scales for depth */\n"
    "    float thresh = 1.0 - u_density * density_mod;\n"
    ...
    "\n"
    "    /* Color temperature variation: warm near galactic center, cool at poles */\n"
    "    float warmth = (1.0 - gal_lat) * 0.15;\n"
    "    stars *= vec3(1.0 + warmth, 1.0, 1.0 - warmth * 0.5);\n"
    "\n"
