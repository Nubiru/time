...
void zodiac_pass_draw(const render_frame_t *frame) {
    ...
    glUniform1f(s_ring_loc_opacity, ring_opacity);

    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);

    glBindVertexArray(s_ring_vao);
    glDrawElements(GL_TRIANGLES, s_ring_index_count,
                    GL_UNSIGNED_INT, 0);
    glBindVertexArray(0);
    ...
    /* --- Planet markers as small spheres on the ring --- */
    ...
    glUniform1f(s_loc_opacity, ring_opacity);
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    ...
    /* --- Aspect lines as colored chords inside the ring --- */
    {
        ...
        if (alines.count > 0) {
            ...
            glUniform1f(s_line_loc_opacity, ring_opacity);
    ...
    /* --- House cusp lines as radial lines across the ring --- */
    {
    ...
    /* --- Zodiac sign labels as billboarded glyph quads --- */
    {
        ...
        glUniform1f(s_glyph_loc_opacity, ring_opacity);
