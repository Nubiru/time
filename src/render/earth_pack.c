...
const char *erp_atmo_frag_source(void)
{
    return
        ...
        "\n"
        "void main() {\n"
        "    // Atmosphere color: soft blue\n"
        "    vec3 atmo_color = vec3(0.3, 0.6, 1.0);\n"
        "\n"
        "    // Limb glow: bright at edges (grazing angle), transparent at center\n"
        "    float vdn = abs(dot(normalize(v_view_dir), normalize(v_normal)));\n"
        "    float limb = 1.0 - vdn;\n"
        "    float glow = limb * limb * limb; // cubic falloff\n"
        "\n"
        "    float alpha = glow * v_opacity * 0.8;\n"
