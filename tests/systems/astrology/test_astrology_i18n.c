...
{
    astrology_interp_t ru = ai_interpret_locale(0, 1, -1, I18N_LOCALE_RU);
    /* RU has no content table — falls back to EN */
    TEST_ASSERT_EQUAL_STRING("Ari", ru.glyph);
    TEST_ASSERT_NOT_NULL(strstr(ru.glance, "Aries"));
    TEST_ASSERT_NOT_NULL(strstr(ru.glance, "Taurus"));
}
