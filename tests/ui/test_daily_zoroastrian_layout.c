...
#include "../../src/ui/daily_zoroastrian_layout.h"
#include <string.h>
...
{
    daily_zoroastrian_layout_t layout = daily_zoroastrian_compute(TEST_JD);
...
void test_is_gatha_bool(void)
{
    daily_zoroastrian_layout_t layout = daily_zoroastrian_compute(TEST_JD);
    TEST_ASSERT_TRUE(layout.is_gatha == 0 || layout.is_gatha == 1);
}
...
{
    daily_zoroastrian_layout_t layout = daily_zoroastrian_compute(TEST_JD);
    TEST_ASSERT_TRUE(layout.festival >= 0 && layout.festival < ZORO_FEST_COUNT);
}
...
void test_gatha_slot(void)
{
    daily_zoroastrian_layout_t layout = daily_zoroastrian_compute(TEST_JD);
    TEST_ASSERT_FLOAT_WITHIN(0.001f, 0.10f, layout.gatha_slot.x);
    TEST_ASSERT_FLOAT_WITHIN(0.001f, 0.75f, layout.gatha_slot.y);
}
...
void test_all_slots_same_width(void)
{
    ...
    TEST_ASSERT_FLOAT_WITHIN(0.001f, 0.80f, layout.gatha_slot.w);
}
...
{
    daily_zoroastrian_layout_t layout = daily_zoroastrian_compute(TEST_JD);
    int f = daily_zoroastrian_has_festival(&layout);
    TEST_ASSERT_TRUE(f == 0 || f == 1);
}

void test_has_festival_null(void)
{
    TEST_ASSERT_EQUAL_INT(0, daily_zoroastrian_has_festival(NULL));
}
...
{
    daily_zoroastrian_layout_t a = daily_zoroastrian_compute(TEST_JD);
    daily_zoroastrian_layout_t b = daily_zoroastrian_compute(TEST_JD);
...
{
    daily_zoroastrian_layout_t a = daily_zoroastrian_compute(2460676.5);
    daily_zoroastrian_layout_t b = daily_zoroastrian_compute(2460706.5);
