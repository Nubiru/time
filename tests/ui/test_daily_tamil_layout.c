...
#include "../../src/ui/daily_tamil_layout.h"
#include <string.h>
...
{
    daily_tamil_layout_t layout = daily_tamil_compute(TEST_JD);
...
{
    daily_tamil_layout_t layout = daily_tamil_compute(TEST_JD);
    TEST_ASSERT_TRUE(layout.date.day >= 1 && layout.date.day <= 32);
}
...
void test_jovian_in_range(void)
{
    daily_tamil_layout_t layout = daily_tamil_compute(TEST_JD);
    TEST_ASSERT_TRUE(layout.date.jovian >= 1 && layout.date.jovian <= 60);
}

void test_jovian_name_not_null(void)
{
    daily_tamil_layout_t layout = daily_tamil_compute(TEST_JD);
    TEST_ASSERT_NOT_NULL(layout.jovian_name);
    TEST_ASSERT_TRUE(strlen(layout.jovian_name) > 0);
}
...
{
    daily_tamil_layout_t layout = daily_tamil_compute(TEST_JD);
    TEST_ASSERT_TRUE(layout.festival >= 0 &&
                     layout.festival < TAMIL_FEST_COUNT);
}
...
void test_jovian_slot(void)
{
    daily_tamil_layout_t layout = daily_tamil_compute(TEST_JD);
    TEST_ASSERT_FLOAT_WITHIN(0.001f, 0.10f, layout.jovian_slot.x);
    TEST_ASSERT_FLOAT_WITHIN(0.001f, 0.47f, layout.jovian_slot.y);
}
...
void test_all_slots_same_width(void)
{
    ...
    TEST_ASSERT_FLOAT_WITHIN(0.001f, 0.80f, layout.jovian_slot.w);
...
{
    daily_tamil_layout_t layout = daily_tamil_compute(TEST_JD);
    int f = daily_tamil_has_festival(&layout);
    TEST_ASSERT_TRUE(f == 0 || f == 1);
}

void test_has_festival_null(void)
{
    TEST_ASSERT_EQUAL_INT(0, daily_tamil_has_festival(NULL));
}
...
{
    daily_tamil_layout_t a = daily_tamil_compute(TEST_JD);
    daily_tamil_layout_t b = daily_tamil_compute(TEST_JD);
...
{
    daily_tamil_layout_t a = daily_tamil_compute(2460676.5);
    daily_tamil_layout_t b = daily_tamil_compute(2460706.5);
...
int main(void)
{
    ...
    /* Jovian cycle */
    RUN_TEST(test_jovian_in_range);
    RUN_TEST(test_jovian_name_not_null);
    /* Festival */
